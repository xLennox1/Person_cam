...
function getDB(){
  ...
  dbPromise=new Promise((resolve,reject)=>{
    ...
    req.onupgradeneeded=e=>{ const db=e.target.result; if(!db.objectStoreNames.contains(STORE_VIDEOS)) db.createObjectStore(STORE_VIDEOS,{keyPath:'id'}); if(!db.objectStoreNames.contains(STORE_FACES)) db.createObjectStore(STORE_FACES,{keyPath:'id'}); };
...
function loadSettings(){
  ...
  try{ const raw=localStorage.getItem('personcam_settings'); if(raw) s={...s,...JSON.parse(raw)}; }catch(e){ console.error('Einstellungen konnten nicht geladen werden',e); }
  graceInput.value=s.grace; sensitivityInput.value=s.sensitivity; trackingEnabledInput.checked=s.trackingEnabled; faceSnapshotsEnabledInput.checked=s.faceSnapshotsEnabled; faceMinConfidenceInput.value=s.faceMinConfidence; faceCooldownInput.value=s.faceCooldown;
}
function saveSettings(){
  const s={grace:Number(graceInput.value)||4,sensitivity:Number(sensitivityInput.value)||55,trackingEnabled:trackingEnabledInput.checked,faceSnapshotsEnabled:faceSnapshotsEnabledInput.checked,faceMinConfidence:Number(faceMinConfidenceInput.value)||70,faceCooldown:Number(faceCooldownInput.value)||5};
  try{ localStorage.setItem('personcam_settings',JSON.stringify(s)); }catch(e){ console.error('Einstellungen konnten nicht gespeichert werden',e); }
}
...
function getModelErrorMessage(error){
  ...
  if(/failed to fetch|network|load|404|cors|internet/i.test(text)) return 'KI-Modelle konnten nicht aus dem Internet geladen werden. Bitte Internetverbindung prüfen und erneut versuchen.';
  if(/webgl|shader|texture|backend/i.test(text)) return 'Die Grafikbeschleunigung dieses Geräts konnte nicht verwendet werden. Bitte erneut versuchen.';
...
async function prepareTensorFlow(){
  ...
  try{
    ...
    catch(cpuError){ throw new Error(`TensorFlow-Backend konnte nicht gestartet werden: ${cpuError?.message||cpuError}`); }
  }
}

async function loadModels(){
  await prepareTensorFlow();
  // Modelle nacheinander laden, damit Mobilgeräte nicht gleichzeitig zu viel RAM benötigen.
...
async function startCamera(){
  ...
  try{
    ...
    console.error(e); let msg='Bitte erlaube den Kamera-/Mikrofonzugriff. Die Seite muss in der Regel über HTTPS oder localhost laufen.';
    if(e.name==='NotFoundError')msg='Keine Kamera gefunden.'; else if(e.name==='NotReadableError')msg='Die Kamera wird bereits von einer anderen App verwendet.'; else if(e.name==='NotAllowedError')msg='Kamera-/Mikrofonzugriff wurde verweigert.'; else if(e.name==='SecurityError')msg='Kamera darf auf dieser Seite nicht verwendet werden. Bitte HTTPS verwenden.'; else if(e.message)msg=e.message;
    setStatus('Kamera konnte nicht gestartet werden'); alert(msg); return;
  }
  video.srcObject=stream; await video.play(); running=true; startBtn.disabled=true; stopBtn.disabled=false;
  try{
    setStatus('Kamera läuft – KI-Modelle werden geladen…');
    await loadModels(); personStreak=0;detecting=false;consecutiveErrors=0;personTracker.reset();faceTracker.reset();faceSnapshotTimes.clear();
    setStatus('Bereit – Personen werden erkannt'); detectLoop();
  }catch(e){
    console.error('KI-Modelle konnten nicht geladen werden:',e); running=false;startBtn.disabled=false;stopBtn.disabled=true;stream?.getTracks().forEach(t=>t.stop());stream=null;video.srcObject=null;setStatus('KI-Modelle konnten nicht geladen werden');alert(getModelErrorMessage(e));
  }
}
...
async function detectLoop(){
  if(!running||!cocoModel||!faceModel)return;
  const now=performance.now(),newFrame=video.readyState>=2&&video.currentTime!==lastVideoTime;
  if(newFrame&&!detecting&&(now-lastDetectAt)>=DETECT_INTERVAL_MS){
    lastVideoTime=video.currentTime;lastDetectAt=now;detecting=true;
    try{
      const frameW=video.videoWidth||1280,frameH=video.videoHeight||720,threshold=getThreshold();
      const [predictions,faces]=await Promise.all([cocoModel.detect(video),faceModel.estimateFaces(video,false)]);
      const people=predictions.filter(p=>p.class==='person'&&p.score>=threshold&&boxIsBigEnough(p.bbox,frameH));
      const faceBoxes=faces.map(f=>{const [x1,y1]=f.topLeft,[x2,y2]=f.bottomRight;const score=Array.isArray(f.probability)?f.probability[0]:f.probability;return{bbox:[x1,y1,x2-x1,y2-y1],score};}).filter(f=>f.score>=getFaceMinConfidence());
      const trackedPeople=trackingEnabledInput.checked?personTracker.update(people.map(p=>p.bbox),frameW,frameH):people.map(p=>({box:p.bbox,id:null}));
      const trackedFaces=faceTracker.update(faceBoxes.map(f=>f.bbox),frameW,frameH);
      drawHud(people,trackedPeople,faceBoxes,trackedFaces); maybeCaptureFaceSnapshots(faceBoxes,trackedFaces);
      if(people.length){if(stopTimer){clearTimeout(stopTimer);stopTimer=null;}personStreak++;if(!recording){if(personStreak>=CONFIRM_HITS)startRecording();else setStatus(`Person erkannt – bestätige… (${personStreak}/${CONFIRM_HITS})`);}}
      else{if(personStreak>0&&!recording)setStatus('Bereit – Personen werden erkannt');personStreak=0;if(recording&&!stopTimer){const seconds=Math.max(1,Number(graceInput.value)||4);stopTimer=setTimeout(()=>{stopTimer=null;stopRecording();},seconds*1000);}}
      consecutiveErrors=0;
    }catch(err){console.error('Erkennung fehlgeschlagen:',err);consecutiveErrors++;if(consecutiveErrors>=MAX_CONSECUTIVE_ERRORS)setStatus('Erkennung läuft instabil – Seite bei Bedarf neu laden');}
...
async function maybeCaptureFaceSnapshots(faceBoxes,trackedFaces){
  ...
  const now=Date.now(),cooldown=getFaceCooldownMs();
  for(let i=0;i<faceBoxes.length;i++){
    const tracked=trackedFaces[i],id=tracked?.id??`face-${i}`,last=faceSnapshotTimes.get(id)||0;if(now-last<cooldown)continue;
    const f=faceBoxes[i],[x,y,w,h]=f.bbox,padX=w*0.35,padY=h*0.45,sx=Math.max(0,Math.floor(x-padX)),sy=Math.max(0,Math.floor(y-padY)),ex=Math.min(video.videoWidth,Math.ceil(x+w+padX)),ey=Math.min(video.videoHeight,Math.ceil(y+h+padY)),sw=Math.max(1,ex-sx),sh=Math.max(1,ey-sy);
    const crop=document.createElement('canvas');crop.width=sw;crop.height=sh;const cropCtx=crop.getContext('2d');cropCtx.drawImage(video,sx,sy,sw,sh,0,0,sw,sh);
    try{const blob=await new Promise(resolve=>crop.toBlob(resolve,'image/jpeg',0.9));if(!blob)continue;const record={id:`face-${Date.now()}-${Math.random().toString(36).slice(2,8)}`,blob,createdAt:Date.now(),confidence:f.score};await idbPut(STORE_FACES,record);faceSnapshotTimes.set(id,now);await renderFaces();}catch(e){console.error('Gesichtsbild konnte nicht gespeichert werden:',e);}
  }
}
